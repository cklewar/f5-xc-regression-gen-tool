...
variables:
  ...
  {% for provider, values in providers -%}
  {% for rte in values.rtes -%}
  {% for rte_provider, rte_values in rte.provider -%}
  {% for v in rte_values.variables -%}
  {% if provider == rte_provider -%}
  RTE_{{ rte.name | upper }}_{{ provider | upper }}_{{ v.name | upper }}: "$ARTIFACTS_ROOT_DIR/rte_{{ rte.name }}_{{ v.value }}"
  {% endif -%}
  {% endfor -%}
  {% endfor -%}
  {% endfor -%}
